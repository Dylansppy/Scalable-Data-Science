...
def mapper(line):
    ...
    count = defaultdict(int)  # has default initial value of 0 for counting
    count[line] = 1  # count for line

    return list(count.items())  # return list of tuples only
...
def map_input_split(input_split):
    ...
    for line in input_split:
        results.extend(mapper(line))  # apply mapper function to line
...
if __name__ == "__main__":
    ...
    for (line, count) in results:
        ...
        total_line += count
    print(" ")
    print("Total line count:", total_line)
